...
const EditsListing = () => {
    ...
    useEffect(()=>{
        ...
        async function fetchListing(){
           ...
           if(data.length =1){
            ...
            toast.error('sorry listing does not exist')
            navigate('/')
           }
    ...
    useEffect(()=>{
        if(listing && usering.id !== listing.user_id){
            console.log()
            toast.error('you cannot edit this list sorry')
            navigate('/')
            return 
        }  
    ...
    const onSubmit = async (e) => {
        ...
        try {
           ...
           // If everything is successful, update the formData state with the image URLs
            ...
            .update([formDataCopy], { returning: "representation" })
            .eq('id',params.listingId)
            .select()
            console.log(data)
            if (error) throw error
    ...
    return (
        <main className='max-w-md px-2 mx-auto'>
            ...
            <form onSubmit={onSubmit}>
                ...
                <input
                    placeholder='Name' type='text' id='name' value={name} onChange={onChange} maxLength='32' minLength='10' required
                ...
                <textarea
                    placeholder='Address' type='text' id='address' onChange={onChange} required value={address}
                    className='w-full px-4 py-2 text-xl text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border-slate-600 mb-6'
                />

                <p className='text-lg mt-6 font-semibold'>Description</p>
                <textarea
                    placeholder='Description' type='text' id='description' onChange={onChange} value={description} required
                    className='w-full px-4 py-2 text-xl text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border-slate-600 mb-6'
                />
